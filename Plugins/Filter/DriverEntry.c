...
static NTSTATUS AttachDiskFilters(PDRIVER_OBJECT DriverObject)
{
    ...
    POBJECT_TYPE* pType = (POBJECT_TYPE*)MmGetSystemRoutineAddress(&(UNICODE_STRING)RTL_CONSTANT_STRING(L"IoDriverObjectType"));
    if (!pType || !*pType) 
...
{
    POBJECT_TYPE* pDevType = (POBJECT_TYPE*)MmGetSystemRoutineAddress(&(UNICODE_STRING)RTL_CONSTANT_STRING(L"IoDeviceObjectType"));
    if (!pDevType || !*pDevType) 
        return STATUS_NOT_SUPPORTED;

    for (ULONG i = 0; g_AttachDisk[i] && g_MbrFilterCount < MAX_MBR_TARGETS; ++i) {
        UNICODE_STRING name; RtlInitUnicodeString(&name, g_AttachDisk[i]);
        PDEVICE_OBJECT fsCtl = NULL;
        if (!NT_SUCCESS(ObReferenceObjectByName(&name, OBJ_CASE_INSENSITIVE, NULL, 0, *pDevType, KernelMode, NULL, (PVOID*)&fsCtl)) || !fsCtl) 
            continue;

        PDRIVER_OBJECT fsDrv = fsCtl->DriverObject;
        if (fsDrv) {
            ULONG need = 0;
            if (IoEnumerateDeviceObjectList(fsDrv, NULL, 0, &need) == STATUS_BUFFER_TOO_SMALL || need) {
                PDEVICE_OBJECT* list = (PDEVICE_OBJECT*)ExAllocatePool2(POOL_FLAG_NON_PAGED, need * sizeof(PDEVICE_OBJECT), 'fsTA');
                if (list) {
                    if (NT_SUCCESS(IoEnumerateDeviceObjectList(fsDrv, list, need * sizeof(PDEVICE_OBJECT), &need))) {
                        for (ULONG j = 0; j < need && g_MbrFilterCount < MAX_MBR_TARGETS; ++j) {
                            PDEVICE_OBJECT lower = list[j], filter, attached = NULL;
                            if (!NT_SUCCESS(IoCreateDevice(DriverObject, 0, NULL, lower->DeviceType, lower->Characteristics, FALSE, &filter)))
                                continue;
                            
                            if (!NT_SUCCESS(IoAttachDeviceToDeviceStackSafe(filter, lower, &attached)) || !attached) { 
                                IoDeleteDevice(filter); 
                                continue; 
                            }
                            filter->StackSize = lower->StackSize + 1;
...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath)
{
    ...
    if (!NT_SUCCESS(IoCreateDevice(DriverObject, 0, &guardName, FILE_DEVICE_UNKNOWN, 0, FALSE, &g_GuardDevice))) 
    ...
    for (UINT i = 0; i < 50 && !NT_SUCCESS(status); ++i) {
    ...
    t = KeQueryPerformanceCounter(NULL);
    RtlStringCchPrintfW(regAltBuf, RTL_NUMBER_OF(regAltBuf), L"385100.%I64x", t.QuadPart);
    RtlInitUnicodeString(&regAlt, regAltBuf);
    if (NT_SUCCESS(CmRegisterCallbackEx(RegistryProtectCallback, &regAlt, DriverObject, NULL, &g_CmRegHandle, NULL))) 
        g_CmRegActive = TRUE;
