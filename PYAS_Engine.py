import os, re, yara, time, math, json, zlib, numpy
    ...
    try:
        for start in range(0, size, batch_size):
            chunk = numpy.frombuffer(
                view[start:min(start + batch_size, size)], dtype=numpy.uint8
            )
...
def _accumulate_string_chunk(chunk, array, state, pattern):
    ...
    if first_separator:
        state["pending_counts"] += numpy.bincount(
