...
NTSTATUS FileProtectDispatch(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
    ...
    if (stack->MajorFunction == IRP_MJ_POWER) {
        ...
        IoSkipCurrentIrpStackLocation(Irp);
        NTSTATUS st = PoCallDriver(lower, Irp);
        ExReleaseRundownProtection(&g_Rundown);
        return st;
    }
    ...
    if (!is_white) {
        ...
        if (isDisk) {
            if (mj == IRP_MJ_WRITE || mj == IRP_MJ_SET_INFORMATION || mj == IRP_MJ_DEVICE_CONTROL || mj == IRP_MJ_INTERNAL_DEVICE_CONTROL) blocked = TRUE;
        }
    }
    if (blocked) {
        if (KeGetCurrentIrql() == PASSIVE_LEVEL) {
